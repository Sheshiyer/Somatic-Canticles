...
This folder now stages a narrow Book 3 serial opening: `Chapter 16-19` are active prose lanes, and nothing later is open.

Current scope:

- `packets/` remain open for `Chapter 16-19`
- `working/` may contain `Chapter 16-19` only
...
- `Chapter 19` now has an active `working/` copy under triangulation-specific constraints and is baseline-ready for compiled comparison
- keep `Chapter 20-27` in the intake and projection surfaces until the remaining gates are closed
...
- `Chapter 16-19` are the only active prose lanes because the delta ledger now fences the main risks: compiled scaffolding contamination, `Chapter 18` exposition collapse, `15 -> 16` hinge softness, Gardener over-mechanization, and witness/vector lexicon drift
...
- `Chapter 19` is open only because the recorded `Chapter 18` deltas proved the lane can carry exact vector logic forward, and it is now baseline-ready for compiled comparison before anything later opens
- `Chapter 20-21` remain packet-closed beyond this narrow opening
