...
- [ ] Decide whether Packet `06` needs a `working/` copy or can stay packet-only while the patterned-captivity lane settles
- [ ] If Packet `06` opens, preserve the logic-versus-feeling conflict as method tension that resolves through signal recognition, not through a soft philosophical compromise
...
- `06_WORKBENCH/SC_STORYOPS/story/chapters/book_1_anamnesis_engine/packets/` now contains starter packets for all eight Book 1 chapters, and `Chapter 01-05` are now promoted into `working/`.
...
- The Book 1 lane now consistently reports `Chapter 01-05` as active working copies, with the diagnostic architecture lane complete and the patterned-captivity lane advancing chapter by chapter.
...
- Verification:
  - the Book 1 lane, packet board, and packet metadata now consistently report `Chapter 01-05` as active working copies
  - Packet `01-03` and the Book 1 packet board now mark the full diagnostic architecture lane as active prose work rather than scaffold-only staging, and Packet `04-05` are now live patterned-captivity lanes
  - the current `Chapter 01-05` working copies resolve on disk and differ from their source chapters only in the intended first-pass edits
  - Packet `01-05` are now baseline-ready for compiled comparison rather than open-ended drafting
